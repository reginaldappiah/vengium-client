package com.jagex;/* Class252 - Decompiled by JODE
...
public class Class252 {
	...
	static void method2409(GraphicsToolkit class_ra, byte[][][] is, int i, int i_0_, int i_1_, int i_2_, int i_3_, int i_4_, int i_5_, int i_6_, int i_7_, int i_8_, byte i_9_) {
		try {
			...
			throw ErrorReporter.generateReport(runtimeexception, new StringBuilder().append("kn.a(").append(')').toString());
		}
	}

	static void method2410(Matrix4f viewMatrix) {
		try {
			Class436.aMatrix4f_5494.method2142(viewMatrix);
		} catch (RuntimeException runtimeexception) {
			throw ErrorReporter.generateReport(runtimeexception, new StringBuilder().append("kn.ab(").append(')').toString());
		}
	}
}
