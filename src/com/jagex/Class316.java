package com.jagex;/* Class316 - Decompiled by JODE
...
public class Class316 implements Runnable {
	...
	Thread aThread3316;
	int anInt3317 = 0;
	public static QuestTypeLoader aClass362_3318;

	void method3839(int i) {
		try {
			this.anInputStream3312 = new InputStream_Sub1();
		} catch (RuntimeException runtimeexception) {
			throw ErrorReporter.generateReport(runtimeexception, new StringBuilder().append("ne.i(").append(')').toString());
		}
	}
	...
	public void run() {
		try {
			...
			throw ErrorReporter.generateReport(runtimeexception, new StringBuilder().append("ne.run(").append(')').toString());
		}
	}
	...
	boolean method3840(int i, int i_1_) throws IOException {
		try {
			...
			throw ErrorReporter.generateReport(runtimeexception, new StringBuilder().append("ne.a(").append(')').toString());
		}
	}
	...
	int method3841(byte i) throws IOException {
		try {
			...
			throw ErrorReporter.generateReport(runtimeexception, new StringBuilder().append("ne.f(").append(')').toString());
		}
	}
	...
	void method3842(int i) {
		try {
			...
			throw ErrorReporter.generateReport(runtimeexception, new StringBuilder().append("ne.p(").append(')').toString());
		}
	}
	...
	int method3843(byte[] is, int i, int i_5_, byte i_6_) throws IOException {
		try {
			...
			throw ErrorReporter.generateReport(runtimeexception, new StringBuilder().append("ne.b(").append(')').toString());
		}
	}
	...
	static final void connect(int key, LinkedList list) {
			if (-1 != key && list.get((long) key) == null)
				list.connect(new Node(), (long) key);
	}
}
